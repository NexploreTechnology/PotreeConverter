...
#ifndef LASPOINTWRITERMODIFIED_H
#define LASPOINTWRITERMODIFIED_H
...
class LASPointWriterModified {

public:
    LASPointWriterModified() {}
    ...
    void writePoints(std::filesystem::path path, AABB aabb, double scale, std::vector<Point> points)
    {
        ...
        for (auto p : points)
        {
            ...
            // get color
            (*point_write).rgb[0] = p.color.x * 256;
            (*point_write).rgb[1] = p.color.y * 256;
            (*point_write).rgb[2] = p.color.z * 256;
